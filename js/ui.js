...
/**
 * Displays a popup.
 * @param {string} html - HTML content of the popup
 * @param {Object} options - Optional settings
 *   options.showOk {boolean} - Whether to show the default OK button (default: true)
 *   options.showGraph {boolean} - Whether to include a canvas for graph drawing (default: false)
 */
...
function trade(stockIndex, amount){
  ...
  if(tradeMode==="buy") buy(stockIndex,amount);
    ...
    else popup("You don't own enough shares.", { showOk: true, showGraph: false });
  }
  renderStockTable();
}
...
function showPlayerInfo(playerIndex){
  ...
  const lastWorth = p.history[p.history.length-1] || 0;
  const changePercent = lastWorth ? (((total-lastWorth)/lastWorth)*100).toFixed(2) : "0";
...
function confirmEndGame() {
  ...
  popup(popupHtml, { showOk: false, showGraph: false });
}
